  ...
  return (
    <main className="min-h-screen bg-[#050505] pt-36 pb-20 px-6">
      <div className="max-w-xl mx-auto text-center bg-[#0A0A0A] border border-white/10 rounded-2xl p-10">
        ...
        <h1 className="text-3xl font-bold text-white mb-4">{checking ? 'Verifying payment' : confirmed ? 'Payment confirmed' : pending ? 'Payment received' : cancelled ? 'Payment cancelled' : 'Payment failed'}</h1>
        <p className="text-gray-400 leading-relaxed">{checking ? 'Please wait while we confirm your payment.' : confirmed ? 'Payment received. Your order is now queued for engineering.' : pending ? 'Your payment was received by Razorpay. Verification is pending, and your order will update automatically after Razorpay confirms it.' : cancelled ? 'The payment was cancelled. Your order remains available if you want to try again.' : 'Razorpay reported that this payment failed.'}</p>
