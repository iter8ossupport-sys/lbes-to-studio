  ...
  return (
    <main className="min-h-screen bg-[#050505] pt-36 pb-20 px-6">
      <div className="max-w-xl mx-auto text-center bg-[#0A0A0A] border border-white/10 rounded-2xl p-10">
        {checking ? <Loader2 size={48} className="mx-auto text-orange-400 mb-6 animate-spin" /> : cancelled ? <XCircle size={48} className="mx-auto text-gray-500 mb-6" /> : confirmed ? <CheckCircle2 size={48} className="mx-auto text-green-400 mb-6" /> : <Clock3 size={48} className="mx-auto text-yellow-400 mb-6" />}
        <p className="text-orange-400 text-xs font-bold tracking-[0.2em] mb-4">PAYMENT STATUS</p>
        <h1 className="text-3xl font-bold text-white mb-4">{checking ? 'Verifying payment' : confirmed ? 'Payment confirmed' : 'Payment not confirmed'}</h1>
        <p className="text-gray-400 leading-relaxed">{checking ? 'Please wait while we confirm your payment.' : confirmed ? 'Payment received. Your order is now queued for engineering.' : "We couldn't confirm your payment yet. Leaving Razorpay does not automatically mean payment failed."}</p>
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          ...
          {!confirmed && paymentLink && <a href={`${paymentLink}?order_id=${encodeURIComponent(orderId || '')}`} className="rounded-xl bg-orange-500 px-6 py-3 font-semibold text-black hover:bg-orange-400">Try Payment Again</a>}
